...
$(window).on("load", function () {
  ...
  if ($(".slider-logo__v2")) {
    new Swiper(".slider-logo__v2", {
      ...
      speed: 3e3,
      preventInteractionOnTransition: !0,
      autoplay: { delay: 1, reverseDirection: !0, disableOnInteraction: !1 },
      breakpoints: {
        540: { slidesPerView: 3 },
        768: { slidesPerView: 4 },
        992: { slidesPerView: 6 },
...
function Load3dCarousel() {
  if ($(".slider-features__sub").length) {
    ...
    let x = showcase.width() / 2; // Get Half Width of Container
    showcase.Cloud9Carousel({
      ...
      mirror: {
        gap: 12, // Distance of items
        height: 0.1, // Height of items
        opacity: 1, // Opacity far from active
      },
      ...
      autoPlay: 0, // Disable autoplay (set to 0)
      autoPlayDelay: 60000, // Delay time (irrelevant since autoPlay is 0)
      speed: 0, // Speed is 0 to stop movement
      bringToFront: true, // Make item active to in front of carousel
      onRendered: function () {
        var item = showcase.data("carousel").nearestItem(); // Get active item
        showcase.find(".thumb").removeClass("active"); // Remove class if not active
        item.element.classList.add("active"); // Add class to make animation
        var c = Math.cos(
          (showcase.data("carousel").floatIndex() % 1) * 2 * Math.PI
        ); // Math opacity text when spin (no effect since it doesn't move)
